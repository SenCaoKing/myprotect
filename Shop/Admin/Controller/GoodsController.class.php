...
class GoodsController extends BaseController{
	...
	public function number(){
		...
		// 先在goods_attr表里取出这件商品同一个属性有多个值得属性id,再在goods_attr表里找出attr_id等于上面id的记录，最后去联attr表查询这个属性的名称
		$sql="select a.attr_name,b.* from ecshop_attr a left join ecshop_goods_attr b on a.id=b.attr_id where b.attr_id in(select attr_id from ecshop_goods_attr where goods_id='$goods_id' group by attr_id having count(*)>1) and goods_id='$goods_id'";
