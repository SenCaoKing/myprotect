...
class GoodsModel extends Model{
	...
	protected $_validate = array(
        ...
        array('seo_description', '1,150', 'seo优化[搜索引擎【百度、谷歌等】优化]_描述的值最长不能超过 120 个字符！', 2, 'length', 3),
     ...
     */
    public function search($pageSize = 10,$is_delete=0){
    	/************ 搜索 ****************/
    ...
    /**
     * 修改前 --- 钩子函数，在修改信息前删除以前的图片以及上传新的图片(&$data 是“引用”传递，函数内部改变之，外边也可以访问到)
     * @param  [type] &$data  [description]
     * @param  [type] $option [description]
     * @return [type]         [description]
     */
    ...
    protected function _before_delete($option){
        ...
        $model = M('GoodsPics');
        // 先取出图片的路径
        $pics = $model->field('pic,sm_pic')->where(array('goods_id'=>array('eq', $option['where']['id'])))->delete();
        // 循环每个图片进行删除
        foreach ($pics as $p) {
            deleteImage($p);
        }
    ...
    public function getPromote($limit=5){
        ...
        $data=$this->field('id,goods_name,promote_price,sm_logo')->where($map)->order('sort_num desc')->limit($limit)->select();
    ...
    public function getNew($limit=5){
        ...
        $data=$this->field('id,goods_name,shop_price,sm_logo')->where($map)->order('sort_num desc')->limit($limit)->select();
        S('new_goods',$data);
        return $data;
    }
